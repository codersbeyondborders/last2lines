...
/*** New Entry **/
	if(isset($_POST['newEntrySubmit'])){
		...
		$line1 = mysql_prep($_POST['line1']);
		
		if(isset($_POST['line2']))
		$line2 = mysql_prep($_POST['line2']);
		
		if(isset($_POST['chapter_id']))
		$chapter_id = mysql_prep($_POST['chapter_id']);
		
		if($name != null && $email!= null && $line1!=null && $line2!=null&& $chapter_id !=null)
			$flag = true;
		else 
			$flag = false;
		...
		if($flag){
			...
			$query = "INSERT INTO current_table (
					...
					) VALUES ('{$email}', '{$name}','{$line1}','{$line2}',{$chapter_id});";
			...
			$contents = $line1.PHP_EOL;
			fwrite($myFileLink2, $contents);
			$newContents = $line2.PHP_EOL;
			fwrite($myFileLink2, $newContents);
			$newContents2 = $name.PHP_EOL;
            ...
            <div id="page-inner">
                ...
                <div class="row">
					...
					<hr/>
						...
						action="dashboardNewEntry.php" method="post"
						>
