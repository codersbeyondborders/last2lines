...
<?php
$archive_chapter_id = null;
$_SESSION['chID']=null;
if(isset($_GET['chID']))
	$archive_chapter_id = $_GET['chID'];
?>
...
if(isset($_POST['submitSearchBtn'])){
	if(isset($_POST['name']) && $_POST['name'] != ""){
		...
		$data = searchDisplay($name,$archive_chapter_id);
		...
		$_SESSION['data']= $data;
		$_SESSION['name'] = $name;
        header("Location:archives.php?chID={$archive_chapter_id}");
       exit;
...
<script>
        $(document).ready(function() {
            ...
            $('input.autosearch').typeahead({
                name: 'name',
                remote: 'search.php?<?php echo "chid={$archive_chapter_id}";?>&query=%QUERY'
...
<div id="poemSummary" class="row">
		...
		<div id="ourJourneySoFar" class="fh5co-counters" style="margin-right:5px;padding: 2em 1em;margin-top:-40px;">
			
			  
			  <?php
				$ch = null;
				$ch = getChapterById($archive_chapter_id);
				
				//Total Count Of Authors
				$count = getEntriesCountFromChapter($ch['chapter_id']);
				$couplets = $count*2;
			  
			  ?>
			  <div class="row to-animate" style="margin-top:-80px; margin-bottom:-80px;">
			  
				
					<div class="col-md-12 ">
						<div class="to-animate"><?php
			  ...
			  ?></div>
					...
					<div class="col-md-offset-2 col-md-3 text-center">
						...
						<span class="fh5co-counter js-counter" data-from="1" data-to="1" data-speed="1000" data-refresh-interval="50">1</span>
						<span class="fh5co-counter-label">Voice</span>
					</div>
					
					<div class="col-md-3 text-center">
						<span class="fh5co-counter js-counter" data-from="0" data-to="<?php echo "{$count}";?>" data-speed="1000" data-refresh-interval="60"></span>
						<span class="fh5co-counter-label">Authors</span>
					</div>
					
					<div class="col-md-3  text-center">
						<span class="fh5co-counter js-counter" data-from="0" data-to="<?php echo "{$couplets}";?>" data-speed="1000" data-refresh-interval="60"></span>
						<span class="fh5co-counter-label">Lines</span>
					</div>
...
<section class="col-md-offset-1 col-lg-offset-1 col-md-7 col-lg-7">
...
<strong style=\"color:#111;\"><br/><br/>.. because without you we cannot make a difference.</strong>
					...
					{echo "Search results for <strong>{$_SESSION['name']}</strong> :";}}
			  ?>
			  </h3>
			  <br/>
...
echo "<br/><h3 class=\"text-muted\">------------------------------</h3><br/><br/>";
...
echo "The Poem: <strong>" . $ch['chapter_name'] . "</strong>";  
...
if(!isset($_SESSION["data"]))
if($fullPoemFlag){
$poemFileName = "poem" .$archive_chapter_id .".txt"; 
...
if ($handle) {
    ...
    while (($line = fgets($handle)) !== false) {
        $line = str_replace("\n", "", $line);
		if($counter%3 == 0)
			echo "<h4><span class=\"text-primary\">"."( ".htmlspecialchars($line).")</span></h4><br/>";
		else 
			echo "<h4>".htmlspecialchars($line)."</h4>";
...
<?php
	...
	*/
	...
	$query = "SELECT COUNT(*) FROM current_table WHERE chapter_id = {$archive_chapter_id}";
	$total_pages = mysqli_fetch_array(mysqli_query($connection ,$query),MYSQLI_NUM);
	$total_pages = $total_pages[0];
	
	/* Setup vars for query. */
	$targetpage = "archives.php?chID={$archive_chapter_id}"; 	//your file name  (the name of this file)
	...
	/* Get data. */
	$sql = "SELECT line1,line2,name FROM $tbl_name WHERE chapter_id = {$archive_chapter_id} ORDER BY dateTime LIMIT $start, $limit";
	...
	*/
	...
	{	
		...
		$tempString .= "\"><a href=\"$targetpage&page=000\">Full Poem</a></li>";
...
<!-- Search -->
<div class="" >
<form action="archives.php<?php echo "?chID={$archive_chapter_id}";?>" method="post" >
...
$chapterArchives = getAllChapters();
foreach($chapterArchives as $chapter){
if($chapter['chapter_id'] != $archive_chapter_id)
